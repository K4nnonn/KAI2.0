...
try:
    ...
    def normalize_account_name(value, allow_passthrough: bool = True):
        return value
...
class ScoringResult:
    ...
    def needs_data(self) -> bool:
        ...
        return self.score is None and self.data_needed is not None
...
class UnifiedAuditEngine:
    ...
    def __init__(
        ...
        self.projection_columns = projection_columns or [
            "Account name",
            "Account",
            "Account type",
            "Customer ID",
        ...
        # Column positions (Excel 1-indexed) for the 11/25 template
        self.COL_CATEGORY = 2  # B
        self.COL_ITEM = 3      # C
        self.COL_RATING = 4    # D
        self.COL_DETAILS = 5   # E
        self.COL_ACTIONS = 6   # F
        self.COL_RATIONALE = 7 # G
        ...
        # Inferred customer IDs discovered from matched rows (supplemental, not required).
    ...
    def _read_csv_with_fallbacks(self, csv_file: Path) -> pd.DataFrame:
        """
        Read a CSV using progressively broader parsing rules.

        SA360/Editor exports vary between UTF-8 and UTF-16, with either comma or tab
        delimiters and zero/two header rows. Medicare’s latest Editor dump uses UTF-16
        with a single header row, which the legacy skiprows=2 logic skipped entirely.
        """
        attempts = [
            {"encoding": "utf-8", "sep": None, "skiprows": 2},
            {"encoding": "utf-16", "sep": "\t", "skiprows": 2},
            {"encoding": "utf-16", "sep": "\t", "skiprows": 0},
            {"encoding": "utf-8", "sep": "\t", "skiprows": 0},
            {"encoding": "utf-8", "sep": None, "skiprows": 0},
        ]
        ...
        for params in attempts:
            try:
                ...
                # Optional projection to reduce memory for huge files
                if self.projection_columns:
                    try:
                        ...
                        if desired:
                            read_kwargs["usecols"] = desired
                    except Exception:
                        pass

                # Chunked read to avoid loading entire multi-GB files
                if self.chunk_size and self.chunk_size > 0:
                    ...
                    try:
                        ...
                        # Fallback to full read if chunking fails
                ...
                # If the previous parser skipped the header, we won't see canonical columns.
                if not _looks_like_structured_export(df):
                    continue

                # Some exports repeat the header row as the first data row. Drop it if detected.
        ...
        if last_error:
            raise last_error
        raise ValueError(f"Unable to parse {csv_file}")
    ...
    def _filter_by_business_unit(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Derive customer IDs from matched rows (adaptive, non-hardcoded).
        ...
        if attempted_filter and not applied_filter and self.strict_filter:
            raise ValueError(f"No rows matched business unit filter for {self.business_unit}.")
    ...
    def load_data(self) -> Dict[str, pd.DataFrame]:
        ...
        for csv_file in csv_files:
            ...
            try:
                ...
                # Normalize key: Strip common SA360 export patterns
                normalized_key = file_key.replace('_report_-_kl2', '').replace('_-_kl2', '').replace('_report', '')

                # Strip parenthetical suffixes like "_(5)", "_(3)" from duplicate exports
                import re
                ...
                df_filtered = self._filter_by_business_unit(df)
    ...
    def _get_ad_schedule_metrics(self) -> Optional[Dict[str, float]]:
        ...
        if schedule_source is None:
            for csv_path in self.data_dir.glob("*campaign*settings*.csv"):
                try:
    ...
    def discover_data_sources(self):
        ...
        self.campaign_source = self.source_info['campaign_attributes_source']
    ...
    def score_criterion(self, excel_row: int, criterion_name: str, category: str) -> ScoringResult:
        ...
        if "attribution" in criterion_lower and ("within ppc" in criterion_lower or "cross-channel" in criterion_lower):
            return ScoringResult(
                score=None,
    ...
    def _score_google_presence(self) -> ScoringResult:
        ...
        def _compute_google_presence(df: pd.DataFrame) -> Optional[Tuple[pd.DataFrame, Optional[str], int, str, bool]]:
            ...
            name_mask = filtered_campaigns['Account name'].str.contains('Fidelity|Google', case=False, na=False)
            if campaign_col:
    ...
    def _score_disapprovals(self) -> ScoringResult:
        ...
        ACCURATE: Only counts disapprovals for ACTIVE keywords.
        Paused keywords are excluded because:
        1. Paused state takes precedence over disapproval
        2. They're not actively serving (no user impact)
        3. Disapproval flag may be stale from when they were active

        Uses Search keyword.csv with status "Not eligible" for true policy violations.
        """
        keyword_data = self.data.get('search_keyword')
        if keyword_data is None:
            return ScoringResult(
                score=None,
                details="Data needed: Search keyword report with status information",
                recommendations="Export search keyword report from SA360",
                data_needed="Search keyword report"
            )

        if 'Search keyword status' not in keyword_data.columns:
            return ScoringResult(
                score=None,
                details="Data needed: Keyword status column not found",
                recommendations="Ensure search keyword export includes status column",
                data_needed="Search keyword status column"
            )

        # Count ACTIVE keywords (exclude Paused and Removed)
        ...
        # Count true policy disapprovals: "Not eligible" status
        # This represents keywords disapproved for policy violations
        ...
        # Scoring based on active keyword disapproval rate
        ...
        # Build details with transparency
    ...
    def _score_consolidation(self) -> ScoringResult:
        ...
        if keyword_col and adgroup_col:
            ...
            return ScoringResult(
                score=None,
                details="Data needed: Landing page report must include Keyword and Ad group columns.",
                recommendations="Ensure landing page export includes keyword and ad group dimensions.",
                data_needed="Landing page report with keywords"
            )
    ...
    def _score_offline_conversions(self) -> ScoringResult:
        ...
        return ScoringResult(
            score=3,
            details="Offline conversion imports critical for financial services",
    ...
    def generate_audit(self, account_name: str, output_path: Path):
        ...
        for excel_row in range(self.FIRST_CRITERION_ROW, self.LAST_CRITERION_ROW + 1):
            ...
            if result.is_scored():
                ...
                ws.cell(excel_row, self.COL_ACTIONS).value = result.recommendations
